...
const ElectronicDetails = () => {
  ...
  useEffect(() => {
    fetch("/Electronic.json")
      .then((res) => res.json())
      .then((data) => {
      const foundelectronic = data.find((p) => p.id == id); 
        if (foundelectronic) {
          setElectronic(foundelectronic);
          setMainImg(foundelectronic.images[0]); 
        }
      });
  }, [id]);
    ...
    {electronic?.images?.map((img, index) => (
      ...
      />
        ...
        <div className="w-[530px] px-6 pt-5">
          ...
          <h3 className="text-xl pt-2 text-black">Brand: {electronic.brand}</h3>
          <h3 className="text-xl pt-2 text-black">Type: {electronic.type}</h3>
          <h3 className="text-xl pt-2 text-black">Material: {electronic.material}</h3>
          <h3 className="text-xl pt-2 text-black">Color: {electronic.color}</h3>
          ...
          <div className="flex gap-1 pt-2">
            <h2 className="text-black pt-0.5">{electronic.rating}</h2>
            ...
            <h2 className="text-black pb-0.5">({electronic.reviews})</h2>
          </div>
        </div>
